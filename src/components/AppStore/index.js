...
class AppStore extends Component {
  ...
  render() {
    ...
    const searchInputResults = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    const filteredList = searchInputResults.filter(
      each => each.category === activeTabId,
    )
    return (
      <div className="container">
        <div className="app-container">
          <h1 className="app-container-heading">App Store</h1>
